...
random.seed(763) #create seed for random number generation 
...
def simulation(hospitals, t_max, mutation_rate, removal_rate, symmetric_transfer_rate, transfer_rate, N):
        ...
        for i in range(0, no_hospitals):
            ...
            for j in range(0, int(trees_per_hospital[i])):
                ...
                while count < haplotypes_per_tree:
                    ...
                    #if t > 0 and proportion_plot[3][0][0][t-1] >= N:
                        #mutation = False
                        
                    if t > 0:
                        prev_iteration_count = proportion_plot[3][0][0][t-1]
                        
                        if prev_iteration_count + mutation_count - death_count >= N:
                            mutation = False
...
test = simulation(hospitals, 250, 0.12, 0.1, True, 0.0, 1000)
