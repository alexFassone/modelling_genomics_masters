...
# -*- coding: utf-8 -*-
"""
Created on Thu Oct  3 13:49:34 2019
...
mutation_log = pd.DataFrame(columns = ('Day', 'Patient', 'Original Hap', 'New Hap'))
#print(mutation_log)
...
def check_infection(current_position, hospital_patients):
    haplotype_infectivity = [0, 0.005, 0.01, 0.01]
    ...
    current_haplotype = int(current_patient[1].split(sep = "H")[1])
        ...
        if limit_check < haplotype_infectivity[current_haplotype]:
            ...
            while self_infect == True:
                ...
                if infected_patient_position != current_position:
                    ...
                    infected_haplotype = int(infected_patient[1].split(sep = "H")[1])
    ...
    for i in range(0, no_hospitals):
        ...
        while j < int(samples_per_hospital[i]):
            ...
            if mutation == True:
                ...
                mutation_temp = pd.DataFrame({'Day':[t],
                                             'Patient':[hospitals[i][j].split(sep = ",")[0]],
                                             'Original Hap':[",".join(hospitals[i][j].split(sep = ",")[1:])],
                                             'New Hap':[",".join(new_patient.split(sep = ",")[1:])]})
    
                #print(mutation_temp)
                
                mutation_log = mutation_log.append(mutation_temp, ignore_index=True)
            ...
            if infection[0] == True:
                ...
                infection_temp = pd.DataFrame({'Day':[t],
                                             'Patient':[hospitals[i][j].split(sep = ",")[0]],
                                             'Infected Patient':[hospitals[i][infection[1]].split(sep = ",")[0]],
                                             'New Infected Hap':[",".join(hospitals[i][infection[1]].split(sep = ",")[1:])]})
    
                #print(mutation_temp)
                
                infection_log = infection_log.append(infection_temp, ignore_index=True)
                
            
            #if a transfer has occurred update the patient label
            if transfer[0] == True:
                
                transfer_temp = pd.DataFrame({'Day':[t],
                                             'Patient':[hospitals[i][j].split(sep = ",")[0]],
                                             'Transfer From':[int(i)],
                                             'Transfer To':[int(transfer[1])]})
                
                transfer_log = transfer_log.append(transfer_temp, ignore_index=True)
